  ...
  double getTotal() {
    ...
    if (couponObj != null) {
      if (couponObj.discountType == "percent") {
        ...
        return subtotal - couponObj.amount;
      }
    } else {
      return subtotal;
    }
    ...
    try {
      ...
      if (ready) {
        ...
        if (json != null) {
          ...
          if (userJson != null) {
            final User user = await Services().getUserInfor(id: User.fromLocalJson(userJson).id);
            address = Address(
                firstName: user.billing.firstName.isNotEmpty ? user.billing.firstName : user.firstName,
                lastName: user.billing.lastName.isNotEmpty ? user.billing.lastName : user.lastName,
                email: user.billing.email.isNotEmpty ? user.billing.email : user.email,
